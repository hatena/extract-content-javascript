...
(function(ns) {
    ...
    if (typeof extractContentTest != 'undefined') {
        var debug = extractContentTest.debug;

        extractContentTest.extractContent = function(d) {
            if (!d.body) return null;

            /* TEST for specified handler */

            if (extractContentTest.only == 'Heuristics') {
...
//             ex.addHandler( ex.factory.getHandler('GoogleAdsence') );
        ...
        extractContentTest.doTest = function() {
            var e = extractContentTest.extractContent(document);
            var b = document.body;
            while (b.firstChild) {
        ...
        if (extractContentTest.auto) {
            extractContentTest.doTest();
        }
    }
})(WWW);
